...
const sampleData = () => {
  ...
  for (let inp of input_pts) {
    ...
    if (0 <= grade && grade <= 0.1) { grade = 0 }  // 10% chance to get 0
    if (0.1 < grade && grade <= 0.3) { grade = 1 } // 20% chance to get 1
    if (0.3 < grade && grade <= 0.6) { grade = 2 } // 30% chance to get 2
    if (0.6 < grade && grade <= 1) { grade = 3 }   // 40% chance to get 3
    document.getElementById(inp.id).value = grade;
    if (grade !== 3) {
      document.getElementById(`${qID}_justification`).value = `Un commentaire pour ${qID}`;
    } else {
      document.getElementById(`${qID}_justification`).value = ``;
    }
    document.getElementById(inp.id).closest('.MuiGrid-container').style.backgroundColor = '';
  }
